"""Independent 2N91 adapter with explicit product service mappings.
...
class ProductAdapter:
    ...
    def entities(self, context):
        ...
        for sid, name in [("seatRing", "座圈开合"), ("filpSwitch", "盖板开合")]:
            if _writable(context, sid, "on"):
                result.append(_opening_button(sid, name))
        for sid, name in [
            ("smallflushSwitch", "小冲"),
            ("bigflushSwitch", "大冲"),
            ("cleaningSwitch", "喷枪清洗"),
        ]:
            result.append(_command_button(context, sid, "on", 1, name))
        result.append(_command_button(context, "stop", "action", 0, "停止清洗与烘干"))
