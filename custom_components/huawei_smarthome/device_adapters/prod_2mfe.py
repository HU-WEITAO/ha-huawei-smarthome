...
def _air_conditioner(
    ...
    def state(device: DeviceContext) -> Mapping[str, Any]:
        is_on = _flag_value(device.value("airKey", "on"))
    ...
    async def set_hvac_mode(
        ...
        if hvac_mode == "off":
            await device.async_send_service("airKey", {"on": 0})
            return
        mode = hvac_to_device.get(hvac_mode)
        if mode is None:
            raise ValueError(f"2MFE unsupported HVAC mode: {data.get('hvac_mode')!r}")
        await device.async_send_service("airKey", {"mode": mode})
        await device.async_send_service("airKey", {"on": 1})
    ...
    metadata: dict[str, Any] = {
        "hvac_modes": ["off", *mode_mapping.values()],
        "min_temp": _TEMP_MIN,
        "max_temp": _TEMP_MAX,
    }
    if fan_mapping:
