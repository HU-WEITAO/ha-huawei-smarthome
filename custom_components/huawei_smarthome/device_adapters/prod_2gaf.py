"""User-contributed protocol for Huawei product 2GAF.

Product: 海雀AI全景摄像头 2.5K版 (deviceModel ``HQ5C``, prodId ``2GAF``).
Profile: https://smarthome-drcn.dbankcdn.com/device/guide/2GAF/2GAF.json

This panoramic camera shares its protocol with the 2G4N pan/tilt model: both
Profiles declare the same eleven services (``carmera``, ``alarmEvent``,
``voip``, ``netInfo``, ``deviceInfo`` and ``visitPoint1``..``visitPoint6``)
with identical characteristics, so the mapping below mirrors ``prod_2g4n.py``.

Verification note: the entity and command mapping was derived from the shared
Profile and validated on a 2G4N.  The 2GAF unit available for testing is
currently offline at a remote location, so the commands have **not** been
re-verified on 2GAF hardware.  They are expected to behave identically because
the Profile is byte-for-byte equivalent in the services it declares, but that
expectation is not a substitute for a real test.
...
def _number(value: Any) -> int | float | None:
    ...
    Alarm fields are declared as ``0/1`` but the cloud can deliver them as
    strings, so an unlabelled value still has to be reduced to a number before
    it can be shown.
    """
...
class Product2GAFAdapter:
    """Keep all 2GAF entity and command choices in this file."""
    ...
    def entities(self, context: DeviceContext) -> tuple[EntitySpec, ...]:
        ...
        if _has_service(profile, _ALARM_SID):
            ...
            # The event entity supplies the missing half: it fires on every
