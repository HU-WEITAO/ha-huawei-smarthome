...
核心服务(本次使用的):
   ...
   alertor.alertorEnable enum RW (0=关闭, 1=打开 警戒开关)

本适配器暴露一个 Home Assistant ``light`` 实体(夜灯), 支持 开关/亮度.
以及一个 ``switch`` 实体(报警器), 支持 开/关.
其余为网关子设备管理/门铃/语音留言, 需要私有配置, 暂不映射。
"""
...
def _alertor_to_ha_switch(value: Any) -> bool | None:
    """Convert alertorEnable enum value to HA switch state."""
    if value is None:
        return None
    if isinstance(value, bool):
        return value
...
async def _alertor_turn_on(context: DeviceContext, _data: Mapping[str, Any]) -> None:
    await context.async_send_service("alertor", {"alertorEnable": 1})


async def _alertor_turn_off(context: DeviceContext, _data: Mapping[str, Any]) -> None:
    await context.async_send_service("alertor", {"alertorEnable": 0})


class Product113EAdapter:
    """113E 豪恩网关适配器：夜灯 light 实体和报警器 switch 实体。"""
    ...
    def entities(self, context: DeviceContext) -> tuple[EntitySpec, ...]:
        ...
        # Alertor switch entity
        if context.has_service("alertor"):
            def alertor_state(device: DeviceContext) -> Mapping[str, Any]:
                return {
                    "is_on": _alertor_to_ha_switch(device.value("alertor", "alertorEnable")),
                }
            ...
            entities.append(
                EntitySpec(
                    ...
                    state=alertor_state,
                    actions={
                        "turn_on": _alertor_turn_on,
                        "turn_off": _alertor_turn_off,
                    },
                )
            )
