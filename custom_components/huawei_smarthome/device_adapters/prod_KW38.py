"""Product adapter for the Huawei SmartLock SE with camera (KW38, 华为智能门锁 SE 猫眼版).

Product: 华为智能门锁 SE 猫眼版 (deviceModel ``AGS-S10``, prodId ``KW38``,
manufacturer 华为).  Profile: https://smarthome-drcn.dbankcdn.com/device/guide/KW38/KW38.json

Sibling of the SmartLock Pro (KW02 / AGS-X10) already covered by this
repository: the two Profiles declare the **same 20 services with the same
characteristics**, including an identical eight-field ``event`` service.  The
entity set, command payloads and event decoding therefore mirror the KW02
adapter; what differs is noted below and in the battery reader.

Differences from the KW02 Profile (all in ``volumeSetting``, which this adapter
does not map):

    KW02: currentRing, endTime, keyVolume, nightModeSwitch, ringVolume,
          startTime, supportedRing, voiceVolume
    KW38: the same eight plus homeGreetVol and warningVolume

Verification status -- read this before trusting the entities:

    This adapter has NOT been verified on a KW38 device.  It was written from
    the Profile plus the KW02 implementation, and the maintainer has no
    AGS-S10 available.  The structural claims above were checked by comparing
    the two Profiles field by field; the runtime behaviour (which services the
    firmware actually pushes) was not.

    The battery source is the one place where that distinction matters, because
    the two known members of this family disagree: KW02's firmware never pushes
    the Profile's ``doorBattery``/``catEyeBattery`` and reports both levels
    through ``batteryManager`` instead, while KW5J reads them from the Profile's
...
# lockStatus/status values declared by this family's Profile (KW02 and KW38 agree).
...
def _event_record(device: DeviceContext) -> Mapping[str, Any]:
    ...
    _LOGGER.debug(
        "KW38 event record: event=%s eventData=%s merged=%s",
...
class ProductKW38Adapter:
    """HUAWEI SmartLock SE with camera (AGS-S10) entity and command choices."""
