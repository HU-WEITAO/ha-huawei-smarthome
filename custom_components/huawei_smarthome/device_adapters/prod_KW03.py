"""User-contributed protocol for Huawei product KW03.

Product: HUAWEI SmartLock  (deviceModel ``AGS-Q10``, prodId ``KW03``).
Profile: https://smarthome-drcn.dbankcdn.com/device/guide/KW03/KW03.json
...
# Every lockStatus value the Profile declares.  AGS-Q10 firmware also reports 7,
...
# spell a bare \"7\" in 门锁状态和, since it describes neither an open nor a shut
...
# The Profile declares 24 = 门内开锁, but AGS-Q10 firmware never sends 24: the
...
# AGS-Q10 firmware accepts that write and then ignores it: the lock itself acks
...
def _arm_state_spec(profile: Mapping[str, Any]) -> EntitySpec:
    """布防状态：userOperation 27 = 开启布防，28 = 解除布防（其余操作沿用上次状态）。"""

    cache: dict[str, Any] = {"armed": None}

    def state(device: DeviceContext) -> Mapping[str, Any]:
        op = _number(_event_record(device).get(_USER_OPERATION_FIELD))
        if op == 27:
            cache["armed"] = True
        elif op == 28:
            cache["armed"] = False
        # 其它操作保持上一次布防状态不变，避免被非布防事件冲掉
        if cache["armed"] is None:
            return {"native_value": None}
        return {"native_value": "开启布防" if cache["armed"] else "解除布防"}
...
class ProductKW03Adapter:
    """HUAWEI SmartLock  (AGS-Q10) entity and command choices."""
    ...
    def entities(self, context: DeviceContext) -> tuple[EntitySpec, ...]:
        ...
        if context.has_service(_EVENT_SID) or context.has_service(_EVENT_DATA_SID):
            ...
            # New arm state sensor
            entities.append(_arm_state_spec(profile))
        return tuple(entities)


ADAPTER = ProductKW03Adapter()